...
const DelayCountUp = (state, { timeout }) => [state, delay(CountUp, { timeout })];
...
app({
  init: [{ count: 0, auto: false, input: "100" }, delay(CountUp, { timeout: 1000 })],
  view: (state) => (
    <div>
      <div>count: {state.count}</div>
      <button onClick={[SetCount, { count: 0 }]}>Reset to 0</button>
      <button onClick={CountUp}>increment</button>
      <button onClick={[DelayCountUp, { timeout: 500 }]}>increment with delay</button>
      ...
      <div>
        ...
        <input value={state.input} oninput={[SetInput, (ev) => ev.currentTarget.value]} />
      </div>
    </div>
  ),
